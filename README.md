...
**真实评估结果**（逐样本 LLM 判断，deepseek-chat，全量 124 条）：

| 指标 | 值 | 说明 |
|------|-----|------|
| **Precision** | **86.8%** | LLM 判为 bug 的样本中 86.8% 是真正的问题 |
| **Recall** | **61.1%** | 标注的 108 个真实问题中 LLM 正确识别了 61.1% |
| **F1 Score** | **0.72** | 综合评价 |
| TP/FP/FN/TN | 66/10/42/6 | 真阳性/误报/漏报/真阴性 |

> 评估方法：对每条标注读取目标行 ±15 行上下文，独立 LLM 判断是否存在 bug。
> 低召回样本集中在 STYLE/PERF 类别（LLM 认为不够严重）和行号偏移问题。
