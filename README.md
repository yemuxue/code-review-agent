...
**真实评估结果**（逐样本 LLM 判断，全量 124 条）：

| 模型 | Precision | Recall | F1 | TP/FP/FN/TN |
|------|-----------|--------|-----|-------------|
| **deepseek-v4-flash** | **98.0%** | **44.4%** | **0.61** | 48/1/60/15 |
| deepseek-chat | 86.8% | 61.1% | 0.72 | 66/10/42/6 |

> 评估方法：对每条标注读取目标行 ±15 行上下文，独立 LLM 判断是否存在 bug。
> **flash 更保守**：Precision 98%（几乎零误报）但 Recall 44%（漏报更多）；
> **chat 更激进**：Recall 61% 但 FP 10 条。生产审查选 flash，安全审计选 chat。
